...
export const range: TileGenerator<RangeTileSelect> = (tileSet: RangeTileSelect, coords: Coordinates) =>
  rangeSimple(tileSet, coords);

export const rangeSimple = (tileSet: RangeTileSelect, coords: Coordinates, includeSelf?: boolean) => {
  const { range } = tileSet;
