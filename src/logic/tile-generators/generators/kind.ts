...
export const kind: TileGenerator<KindTileSelect> = (tileSelect, subject, actionState, isValidTile, initialSearch) => {
  ...
  if (tileSelect.target === 'hex') {
    ...
    matchingTargets = targets.filter((x) => tileSelect.kind === x.contains[0].kind);
  }
