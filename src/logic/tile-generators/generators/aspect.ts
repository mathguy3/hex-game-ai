...
export const aspect: TileGenerator<AspectTileSelect> = (
  ...
  if (tileSelect.target === 'hex') {
    matchingTargets = targets.filter((x) => areEqualShallow(tileSelect.aspect, x.properties));
  } else {
    targets = hexList.filter((x) => x.contains.length);
    matchingTargets = targets.filter((x) => areEqualShallow(tileSelect.aspect, x.contains[0].properties));
  }
