...
export const continueGame = (params: { roomCode: string; user: User }): ActionResponse => {
  const { roomCode, user } = params;
  const request: ActionRequest = { type: 'continue', playerId: gameManager.getPlayerIdForRoom(roomCode, user.userId) };
  if (!roomCode || !user) {
