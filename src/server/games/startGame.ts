...
export const startGame = (params: { roomCode: string; user: User }): ActionResponse => {
  const { roomCode, user } = params;
  const request: ActionRequest = { type: 'start', playerId: gameManager.getPlayerIdForRoom(roomCode, user.userId) };
  if (!roomCode || !user) {
