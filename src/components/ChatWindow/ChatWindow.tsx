...
export const ChatWindow = () => {
  ...
  const [log, setLog] = useState<ChatLog>({ gameId: basicActionState.gameState.gameId, messages: [] });
    ...
    if (event.key === 'Enter') {
      try {
        const { chatLog, users } = await client.chat({
          gameId: basicActionState.gameState.gameId,
  ...
  useEffect(() => {
    let intervalRef;
    if (client && basicActionState.gameState.gameId) {
  ...
  }, [client, basicActionState.gameState.gameId]);
  return (
    <Stack
      display="flex"
      position="fixed"
